...
class Robot_PID():
	def __init__(self):
		...
		self.station_keeping_dis = 1.5 # meters
		self.frame_id = 'map'
		...
		self.pub_goal = rospy.Publisher("goal_point", Marker, queue_size = 1)
                self.pub_twist = rospy.Publisher("x1_velocity_controller/cmd_vel", Twist, queue_size=1)
	...
	def odom_cb(self, msg):
		...
		self.pub_cmd.publish(cmd_msg)
                ...
                cmd_twist.linear.x = pos_output
                cmd_twist.angular.z = ang_output
